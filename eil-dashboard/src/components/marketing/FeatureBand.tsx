...
export function FeatureBand({ feature, delay = 0 }: { feature: MarketingFeature; delay?: number }) {
  ...
  return (
    <MotionReveal
      delay={delay}
      className="group rounded-lg border border-slate-200 bg-white p-5 transition-colors hover:border-slate-300 dark:border-[#1f1f1f] dark:bg-[#050505] dark:hover:border-[#3a3a3a]"
    >
      <div className="flex h-full flex-col">
        ...
        {/* Was a 20px-tall target: text with no padding. -my-1.5 keeps the card
            spacing identical while the link becomes something a thumb can hit. */}
        <Link
          href={`/features/${feature.slug}`}
          className="-my-1.5 mt-[18px] inline-flex items-center gap-2 py-1.5 text-sm font-medium text-white"
        >
          Explore feature
...
export function SellingPoint({ children }: { children: ReactNode }) {
  return (
    <li className="flex gap-3 text-sm leading-6 text-[#d0d0d0]">
      <CheckCircleIcon className="mt-0.5 h-4 w-4 flex-none text-[#8f8f8f]" />
      <span>{children}</span>
    </li>
  );
}
