...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetch(`${workerServiceUrl}/process-research-queue`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      console.error("[cron.process-research-queue] worker call failed", {
        status: response.status,
        payload,
      });
      return NextResponse.json(
        {
          error: "Research queue batch failed.",
    ...
    console.info("[cron.process-research-queue] worker queue batch complete", payload);
    return NextResponse.json({ ok: true, ...payload });
  } catch (error) {
    console.error("[cron.process-research-queue] unexpected failure", {
