...
const cases: Array<[string, RepositoryOperation | null, "complete" | "focused", RepositoryOperation[]?]> = [
  ...
  ["Show topic coverage as a bar chart", null, "complete", ["visualize"]],
  ["Count the exact phrase peer feedback across every paper and visualize the result", "analyze_text", "complete", ["analyze_text", "visualize"]],
  ["อธิบายงานวิจัยทุกเรื่องในคลังนี้", "analyze_each_document", "complete"],
];

async function main() {
  for (const [prompt, operation, scopeMode, operations] of cases) {
    ...
    if (operation) assert.equal(plan.operation, operation, prompt);
    assert.equal(plan.scopeMode, scopeMode, prompt);
    if (operations) {
      operations.forEach((expectedOperation) => {
        assert.ok(plan.operations.includes(expectedOperation), `${prompt}: missing ${expectedOperation}`);
      });
    }
    console.log(JSON.stringify({ prompt, operation: plan.operation, operations: plan.operations, scopeMode: plan.scopeMode, source: plan.source }));
  }
