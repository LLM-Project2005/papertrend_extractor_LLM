-- Repository-aware chat: persistent exact term indexes and Repository naming.
...
ALTER TABLE public.paper_term_index ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  CREATE POLICY "paper_term_index_select_own" ON public.paper_term_index
  FOR SELECT USING (auth.uid() = owner_user_id);
EXCEPTION
  WHEN duplicate_object THEN NULL;
END $$;

-- Rename only when the project does not already contain a Repository folder.
